public class CombinationPermutation {
	...
	//@ ensures \result == fac_spec.spec_factorial(n)/(fac_spec.spec_factorial(r) * fac_spec.spec_factorial(n-r));
		...
		long combin;
		combin=fac.factorial(n) / (fac.factorial(r) * fac.factorial(n - r)); // combin = fac.factorial(n) * (fac.factorial(r) * fac.factorial(n-r)); // combin = fac.factorial(n) / (fac.factorial(r) * fac.factorial(n-r));
		return combin;
	}
