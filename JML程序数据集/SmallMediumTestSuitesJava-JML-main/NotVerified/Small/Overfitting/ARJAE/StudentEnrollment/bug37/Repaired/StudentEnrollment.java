public class StudentEnrollment {
    ...
    private /*@ spec_public @*/ void payTuitionWithCreditCard(int payment) 
    {
        ...
        payment -= penalty;
        if (!validPayment(payment) || 0 < payment) { // if (!validPayment(payment)) { //if (validPayment(payment)) {
            tuitionBalance -= payment;
	} else {
            throw new IllegalArgumentException(lastName); // throw new IllegalArgumentException();
        }
    }
