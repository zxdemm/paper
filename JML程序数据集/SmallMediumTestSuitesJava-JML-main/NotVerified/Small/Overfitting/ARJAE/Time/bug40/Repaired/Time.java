public class Time {
    ...
    public /*@ pure @*/ boolean later_than(Time start) 
    {
        if (this.hour != start.hour) {
            ...
            return this.second > start.second; // return this.minute <= start.minute; //return this.minute > start.minute;
        } else { 
    ...
    private /*@ spec_public pure @*/ Time trustedDifference(Time start, Time stop) 
    {
        ...
        if (temp_second < start.getSecond()) { // if (temp_minute < start.getMinute()) {
            --temp_hour;
            temp_minute += 60;
        }
