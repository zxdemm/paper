public class Inverse {
    ...
    public static boolean Inverse(int[] x, int[] y) {
        if (x.length > y.length) return false; // if (x.length == y.length) return false; // if (x.length != y.length) return false;
        int index = 0;
        ...
        while (index < x.length) {
            if (y != null && x != null) if (x[index] != y[x.length - 1 - index]) { // if (x[index] != y[x.length - 1 - index]) {
                return false;
            } else {
                index = index + 1;
            }
